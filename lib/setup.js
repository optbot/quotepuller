(function() {
  ...
  var serviceRoot = '/usr/local/lib/quichean/quotepuller';
  ...
  function enableDaemon() {
    ...
    serviceFiles.forEach(function(item, i, arr) {
      shell.cp(path.join(__dirname, item), serviceRoot);
    });
    shell.exec('chown ' + user + ':' + user + ' ' + path.join(serviceRoot, '*'));
  }
})();
